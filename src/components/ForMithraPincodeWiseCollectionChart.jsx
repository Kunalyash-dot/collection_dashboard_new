...
function ForMithraPincodeWiseCollectionChart() {
    const [data, setData] = useState(null);
    useEffect(()=>{
        const fetchData = async()=>{
            try {
                const result =await API.get('/api/charts/mithra-pincode-chart')
  ...
  const calculatePercentage = (collected, total) => {
   ...
   const tableData = Object.keys(data).map((pincode) => {
    const { statuses, totalCount } = data[pincode];
...
export default ForMithraPincodeWiseCollectionChart
